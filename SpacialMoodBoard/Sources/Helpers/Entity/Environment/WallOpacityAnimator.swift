...
final class WallOpacityAnimator {
    ...
    private static let wallNames: Set<String> = [
        "leftWall", "rightWall", "frontWall", "backWall",
    ]
    ...
    func applyInitialOpacity(to rootEntity: Entity, rotationAngle: Float) {
        ...
        for child in rootEntity.children {
            guard let modelEntity = child as? ModelEntity,
                Self.wallNames.contains(child.name)
            else {
                continue
            }

            let targetOpacity: Float =
                transparentWalls.contains(child.name) ? 0.0 : 1.0
