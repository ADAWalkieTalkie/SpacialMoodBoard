...
struct RoomEntityBuilder {
  ...
  private func calculateDimensions(from groundSize: GroundSize) -> SIMD3<Float> {
  ...
  private func createWalls(width: Float, height: Float, depth: Float) -> [ModelEntity] {
    ...
    let wallConfigs: [(name: String, scale: SIMD3<Float>, position: SIMD3<Float>)] = [
      (
        name: "frontWall",
    ...
    return wallConfigs.map { config in
      createWall(name: config.name, scale: config.scale, position: config.position)
    }
  }
  
  @MainActor
  private func createWall(name: String, scale: SIMD3<Float>, position: SIMD3<Float>) -> ModelEntity {
