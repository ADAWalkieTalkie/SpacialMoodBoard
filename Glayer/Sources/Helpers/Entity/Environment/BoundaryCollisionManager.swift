...
final class BoundaryCollisionManager {
    ...
    /// 충돌 감지 임계값 (미터)
    private let collisionThreshold: Float = 0.05
    ...
    /// 경계면과의 충돌 감지
    /// - Returns: 충돌한 벽면 이름과 충돌 강도 (0.0 ~ 1.0)
    private func detectBoundaryCollisions(
        ...
        if leftEdge <= movementBounds.minX + collisionThreshold {
            let depth = movementBounds.minX - leftEdge
            collisions["left"] = min(1.0, max(0.0, depth / collisionThreshold))
        }

        if rightEdge >= movementBounds.maxX - collisionThreshold {
            let depth = rightEdge - movementBounds.maxX
            collisions["right"] = min(1.0, max(0.0, depth / collisionThreshold))
        }
        ...
        if bottomEdge <= movementBounds.minY + collisionThreshold {
            let depth = movementBounds.minY - bottomEdge
            collisions["bottom"] = min(1.0, max(0.0, depth / collisionThreshold))
        }

        if topEdge >= movementBounds.maxY - collisionThreshold {
            let depth = topEdge - movementBounds.maxY
            collisions["top"] = min(1.0, max(0.0, depth / collisionThreshold))
        }
        ...
        if frontEdge <= movementBounds.minZ + collisionThreshold {
            let depth = movementBounds.minZ - frontEdge
            collisions["front"] = min(1.0, max(0.0, depth / collisionThreshold))
        }

        if backEdge >= movementBounds.maxZ - collisionThreshold {
            let depth = backEdge - movementBounds.maxZ
            collisions["back"] = min(1.0, max(0.0, depth / collisionThreshold))
        }
